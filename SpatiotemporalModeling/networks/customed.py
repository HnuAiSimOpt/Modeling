...
from tensorflow.keras.layers import Dense, Conv1D, Conv2D, Conv3D, Conv2DTranspose
...
class Selfattention(keras.layers.Layer):
    ...
    def __init__(self,shape):
        ...
        self.input_dim = self.shape[3]
        self.conv_query = ConvSN2D(filters=self.input_dim // 8, kernel_size=[1, 1],strides=1, padding='VALID')
        self.conv_key = ConvSN2D(filters=self.input_dim // 8, kernel_size=[1, 1],strides=1, padding='VALID')
        self.conv_value = ConvSN2D(filters=self.input_dim, kernel_size=[1, 1],strides=1, padding='VALID')
        self.gamma1 = ConvSN2D(filters=self.input_dim, kernel_size=[1, 1],strides=1, padding='VALID')
        self.gamma2 = ConvSN2D(filters=self.input_dim, kernel_size=[1, 1],strides=1, padding='VALID')

    def call(self, inputs, training=None):

        #proj_query
        proj_query = self.conv_query(inputs)
        proj_query = tf.reshape(proj_query, [self.batch_size, -1, self.pixel_num])
        proj_query = tf.transpose(proj_query, (0, 2, 1))

        #proj_key
        proj_key = self.conv_key(inputs)
        proj_key = tf.reshape(proj_key, [self.batch_size, -1, self.pixel_num])
        ...
        #proj_value
        proj_value = self.conv_value(inputs)
        proj_value = tf.reshape(proj_value, [self.batch_size, -1, self.pixel_num])

        #outputs
        atten_ft = tf.matmul(proj_value, attention)
        atten_ft = tf.reshape(atten_ft, [self.batch_size, self.height, self.width, self.input_dim])
        outputs = self.gamma1(atten_ft) + self.gamma2(inputs)
        return outputs

class PGLayer(keras.layers.Layer):
    def __init__(self, channel_num):
        super(PGLayer, self).__init__()
        self.filter_x = self.build_filter_x(channel_num)
        self.filter_y = self.build_filter_y(channel_num)

    def call(self, inputs, training=None):

        dx = self.conv_op(inputs, self.filter_x)
        dy = self.conv_op(inputs, self.filter_y)

        return dx, dy

    def build_filter_x(self, channel_num):
        filter_x = tf.constant([[1, -1]], dtype=tf.float32)
        filter_x = tf.expand_dims(filter_x, axis=-1)
        filter_x = tf.expand_dims(filter_x, axis=-1)
        filter_x = tf.broadcast_to(filter_x, [1, 2, channel_num, 1])
        return filter_x

    def build_filter_y(self, channel_num):
        filter_y = tf.constant([[1], [-1]], dtype=tf.float32)
        filter_y = tf.expand_dims(filter_y, axis=-1)
        filter_y = tf.expand_dims(filter_y, axis=-1)
        filter_y = tf.broadcast_to(filter_y, [2, 1, channel_num, 1])
        return filter_y

    def conv_op(self, inputs, filters):
...
class SNConvLSTM2DCell(DropoutRNNCellMixin, Layer):
  """Cell class for the Spectual Normalized ConvLSTM2D layer
     <Adapted from official source code in tensorflow2.0>
  ...
  def __init__(self,
               ...
               recurrent_dropout=0.,
               **kwargs):
    super(SNConvLSTM2DCell, self).__init__(**kwargs)
  ...
  def build(self, input_shape):
    ...
    self.ur = self.add_weight(shape=tuple([1, self.recurrent_kernel.shape.as_list()[-1]]),
                         initializer=initializers.RandomNormal(0, 1),
                         name='sn_ur',
                         trainable=False)
  ...
  def call(self, inputs, states, training=None):
    ...
    if training in {0, False}:
        ...
        with tf.control_dependencies([self.u.assign(_u)]):
            W_bar = K.reshape(W_bar, W_shape)
    #Spectral Normalization of Recurrent kernel
    Wr_shape = self.recurrent_kernel.shape.as_list()
    Wr_reshaped = K.reshape(self.recurrent_kernel, [-1, Wr_shape[-1]])
    _ur, _vr = power_iteration(Wr_reshaped, self.ur)
    sigma_r=K.dot(_vr, Wr_reshaped)
    sigma_r=K.dot(sigma_r, K.transpose(_ur))
    Wr_bar = Wr_reshaped / sigma_r
    if training in {0, False}:
        Wr_bar = K.reshape(Wr_bar, Wr_shape)
    else:
        with tf.control_dependencies([self.ur.assign(_ur)]):
            Wr_bar = K.reshape(Wr_bar, Wr_shape)
    ...
    (kernel_i, kernel_f,
     kernel_c, kernel_o) = array_ops.split(W_bar, 4, axis=3)
    (recurrent_kernel_i,
     recurrent_kernel_f,
     recurrent_kernel_c,
  ...
  def get_config(self):
    ...
    base_config = super(SNConvLSTM2DCell, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))


@keras_export('keras.layers.ConvLSTM2D')
class SNConvLSTM2D(ConvRNN2D):
  """Spectual Normalized Convolutional LSTM.
     <Adapted from official source code in tensorflow2.0>
  ...
  def __init__(self,
               ...
               recurrent_dropout=0.,
               **kwargs):
    cell = SNConvLSTM2DCell(filters=filters,
    ...
    super(SNConvLSTM2D, self).__init__(cell,
  ...
  def call(self, inputs, mask=None, training=None, initial_state=None):
    ...
    return super(SNConvLSTM2D, self).call(inputs,
                                        mask=mask,
                                        training=training,
  ...
  def get_config(self):
    config = {'filters': self.filters,
              ...
              'dropout': self.dropout,
              'recurrent_dropout': self.recurrent_dropout}
    base_config = super(SNConvLSTM2D, self).get_config()
